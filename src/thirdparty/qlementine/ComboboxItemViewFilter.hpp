...
// Local patch: never call QComboBox::view() from ChildAdded or while the
// current view is being shown/constructed. Use the known view for geometry
// and defer replacement delegate setup until construction has completed.
...
public:
  ...
  bool eventFilter(QObject* watchedObject, QEvent* evt) override {
    switch (evt->type()) {
      // ChildAdded runs inside the replacement view constructor. Asking for
      // comboBox->view() here can recursively construct another popup/view.
      case QEvent::ChildAdded: {
        if (!_delegateUpdatePending) {
          _delegateUpdatePending = true;
          QTimer::singleShot(0, this, [this]() {
            _delegateUpdatePending = false;
...
private:
  bool _delegateUpdatePending{ false };
  QComboBox* _comboBox{ nullptr };
};
} // namespace oclero::qlementine
